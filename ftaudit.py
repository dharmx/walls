...
class Ok(Generic[OkT, ErrT], Result[OkT, ErrT]): 
    def __init__(self, value: OkT): super().__init__(value, True)

class Err(Generic[OkT, ErrT], Result[OkT, ErrT]): 
    def __init__(self, value: ErrT): super().__init__(value, False)


Discrepancy = NamedTuple("T", [
    ("file_path", str), ("incorrect", str), ("correct", str)])
...
def examine_file(file_path: str) -> Result[Optional[Discrepancy], str]:
    ...
    # one used by the filetype library, as it does not recognize alternative
...
def generate_csv_report(
    ...
    try:
        report: list[str] = [f"IncorrectExtension{delimiter}CorrectExtension{delimiter}FilePath", *[
            f'{d.incorrect}{delimiter}{d.correct}{delimiter}{d.file_path}' for d in discrepancies 
        ]]
...
def generate_json_report(
    ...
    try:
        ...
        return Err(f"Encountered an exception while generating JSON report: {e}")
...
def fix_discrepancies(
    ...
    def fix_discrepancy(discrepancy: Discrepancy) -> Result[None, str]:
        try:
            ...
            correct_abs: str = path.join(wrong_spl[0], f'{wrong_nex}.{discrepancy.correct}')
            ...
            if what_if:
                ...
                try:
                    ...
                    return Err(f'Encountered an exception while fixing discrepancy "{discrepancy.file_path}" -> "{discrepancy.correct}": {e}')
        except OSError as e:
            return Err(f"Encountered an exception while fixing discrepancy: {e}")
...
def gather_file_paths(start_path: str) -> Result[list[str], str]:
    return Ok([
        *chain.from_iterable(
            map(lambda x: 
                map(lambda y: 
                    path.join(x[0], y), x[2]), walk(start_path))
        )
    ])
...
if __name__ == "__main__":
    ...
    for index, argument in enumerate(sys.argv):
        ...
        if argument in ("--path", "-p"):
            ...
            try:
                ...
                print(f'Invalid indent value: "{next_argument}"; must be convertible to an integer.')
                print(f'Exception: {e}')
                exit(1)
            except Exception as e:
                print(f'Unexpected exception when converting "{next_argument}" to an integer: {e}')
    ...
    if output is None:
        print(f'No output was generated for format {output_format}. This could be a bug.')
        exit(1)
    ...
    try:
        ...
        print(f'Encountered an exception while writing to output file {output_file}: {e}')
        exit(1)

    # This should be the very last operation, since it has the ability to modify
    # file names. Keeping it last ensures that any errors will have occured before
